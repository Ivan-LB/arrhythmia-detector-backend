...
### 2026-07-06 — Retire the original pre-rebuild PyQt app

Phase 5 had deliberately left this unresolved (see the entry above): the legacy `Models/*.h5`/`.pk1` binaries, `Data/RawData/`, and `Data/ecg_features3.csv` stayed tracked because they were still load-bearing for the original `UI/`/`ModelCreation/` PyQt app, which was being kept as-is until the new web frontend reached real functional parity. With Phase 4 done and browser-verified, that condition is now met, and retiring the old app was an explicit user decision, not a unilateral call.

Removed on branch `retire-legacy-ui` (off `v2.0.0`): `UI/` (the PyQt app), `ModelCreation/` (old training scripts — `ANNModel.py`, `ANNModel_Prueba.py`, `ModelPreparation.py`; `sineWave.py` was already removed in Phase 5), `Images/` (24 report/evaluation plots from the original project, confirmed unreferenced anywhere), the legacy `Models/*.h5`/`.pk1` binaries (6 files), `Data/RawData/` (32 legacy CSVs), and `Data/ecg_features3.csv`.

Verified before deleting, not assumed: repo-wide grep confirmed none of `ecg_pipeline/`, `training/`, `api/`, or `tests/` referenced any of these paths. Among the deleted files themselves, only `UI/ecg_feature_extractor.py` had a real dependency on the legacy `Models/` binaries (hardcoded absolute paths to `Models/modelo_ecg_*V2.h5`/`scaler_ecgV2.pk1`). `ModelCreation/ANNModel.py` and `ANNModel_Prueba.py` reference CSV filenames matching `Data/RawData/`'s contents, but via hardcoded machine-specific absolute paths from a different computer entirely (e.g. `C:\Users\XPG\Desktop\...`) that never resolved in this repo as committed — so "load-bearing" overstates it; these were already-broken, orphaned scripts that happened to share filenames with the vendored CSVs, not functioning current dependencies. Care taken with `Models/`: only the 6 tracked legacy files were removed by name, not the directory recursively, since the real current model artifact (`models/beat-classifier-20260705-dc9f983/`) physically lives in the same folder on this machine's case-insensitive filesystem (APFS folds `Models/` and `models/` together) despite being a separate, gitignored, untracked path. Confirmed the real artifact survived untouched after the deletion.

(A first pass at this paragraph named the wrong file — `UI/app.py` instead of `UI/ecg_feature_extractor.py` — and overstated the RawData dependency as "load-bearing." Caught by directly grepping each deleted file's last-committed content rather than trusting the first-pass summary; corrected here rather than left wrong.)
